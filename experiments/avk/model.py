...
def load_dataset(filepath: str) -> pd.DataFrame:
    ...
    cols = ['hoursWorkedMean', 'NYT_ConfirmedCases.data', 'NYT_ConfirmedDeaths.data', 'NYT_ConfirmedDeaths.missing',
    ...
    cols_to_normalize = [
        ...
        'unitsInStructure', 'healthInsTotal',
        ...
        'longitude']
    cols_raw = ['dates','fips', 'JHU_ConfirmedCases.data', 'JHU_ConfirmedDeaths.data', 'cyclical_sin', 'cyclical_cos',
                'continuous_sin',
...
def xy_generator(data, fips, days=31, include_fips=False):
...
def prepare_data(df, days_of_history=30, days_to_predict=1, n_samples=200, output_path='./data',include_fips=False):
    fips = df.fips.unique()

    Xi = []
    j = 0

    for i, x in enumerate(xy_generator(df, fips, days=days_of_history+days_to_predict, include_fips=include_fips)):
        Xi.append(x)
